    ...
    try {
      ...
      return await this.sqlServerService.transaction<ReadingResponse>(
        async (conn) => {
          ...
          const insertQuery = `INSERT INTO AP_LECTURAS (Sector, Cuenta, Anio, Mes, LecturaAnterior, LecturaActual, CodigoIngresoARentas, Novedad, ValorAPagar, TasaAlcantarillado, Reconexion, Cod_ingreso, FechaCaptura, HoraCaptura, ClaveCatastral) VALUES (${Number(reading.getSector())}, ${Number(reading.getAccount())}, '${String(reading.getYear())}', '${String(reading.getMonth())}', ${Number(reading.getPreviousReading())}, ${Number(reading.getCurrentReading())}, ${reading.getRentalIncomeCode() != null ? Number(reading.getRentalIncomeCode()) : 'NULL'}, '${String(reading.getNovelty())}', ${reading.getReadingValue() != null ? parseFloat(reading.getReadingValue()!.toFixed(8)) : 'NULL'}, ${reading.getSewerRate() != null ? parseFloat(reading.getSewerRate()?.toFixed(8)!) : 'NULL'}, ${reading.getReconnection() != null ? parseFloat(reading.getReconnection()?.toFixed(8)!) : 'NULL'}, ${reading.getIncomeCode() != null ? Number(reading.getIncomeCode()) : 'NULL'}, '${formattedDate}', '${String(reading.getReadingTime())}', '${String(reading.getCadastralKey())}')`;
